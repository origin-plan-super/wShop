...
<body>
    ...
    <script type="text/html" id="bar1">
        <a class="layui-btn layui-btn-sm" lay-event="carousel">管理轮播</a>
        ...
        layui.use(['table', 'form'], function () {
            ...
            tableIns = table.render({
                ...
                , cols: [[ //表头s
                    ...
                    , { field: 'nav_id', title: 'id', width: 100 }
                    , { field: 'nav_title', title: '导航', width: 150, edit: 'text' }
            ...
            table.on('tool(table_filter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'carousel') {
                    //管理轮播图
                    window.open('/wShop/index.php/Admin/carousel/showList/nav_id/' + data.nav_id);
                }
                    ...
                    for (var j = 0; j < arr.length; j++) {
                        ...
                        if (j != i) {
                            if (item_i['nav_title'] == item_j['nav_title']) {
                                //检测到重名
                                layer.msg('导航名：【' + item_i['nav_title'] + '】检测到冲突，请修改后再操作！');
                                layer.close(load);
                                return;
                            }
                        }
                            ...
                            for (var i = 0; i < arr.length; i++) {
                                ...
                                for (var j = 0; j < nav_arr.length; j++) {
                                    ...
                                    if (item_i['nav_title'] == item_j['nav_title']) {
                                        ...
                                        w(item_i['nav_title']);
                                        w(item_j['nav_title']);
                                        layer.msg('导航名：【' + item_i['nav_title'] + '】已存在，请修改后再操作！');
                                        layer.close(load);
                                        return;
                                    }
                                }
